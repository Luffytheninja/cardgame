...
class Game:
    ...
    def seed_deck(self) -> None:
        base = [
            Card("Strike (Ogun)", 1, "Attack", "Deal 6 damage.", ["I"]),
            Card("Strike (Ogun)", 1, "Attack", "Deal 6 damage.", ["I"]),
            Card("Strike (Ogun)", 1, "Attack", "Deal 6 damage.", ["I"]),
            Card("Defend (Obatala)", 1, "Skill", "Gain 5 Block.", ["II"]),
            Card("Defend (Obatala)", 1, "Skill", "Gain 5 Block.", ["II"]),
            Card("Defend (Obatala)", 1, "Skill", "Gain 5 Block.", ["II"], retain=True),
            Card("Ofunmeji", 1, "Ifa", "Deal 4 damage.", ["I", "II"]),
            Card("Osemeji", 1, "Ifa", "Gain 3 Block.", ["II", "I"]),
        ]
    ...
    def floor_event(self) -> None:
        if self.floor == 5:
            ...
            if choice == "1":
                ...
                self.player.deck.append(Card("Curse", 1, "Curse", "Dead card.", ["II", "II"]))
        ...
        while self.player.hp > 0 and any(e.is_alive() for e in enemies):
            ...
            while True:
                ...
                for i, c in enumerate(self.player.hand, start=1):
                    print(f"{i}. {c.name} [{c.cost}] - {c.description}")
                cmd = input("Play card # or 'end': ").strip().lower()
                if cmd == "end":
                    break
